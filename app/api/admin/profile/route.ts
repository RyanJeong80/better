...
export async function GET(req: NextRequest) {
  ...
  const [dbUser, stats, followerResult, followingResult] = await Promise.all([
    db.query.users.findFirst({
      ...
      columns: { username: true, name: true, avatarUrl: true, country: true, email: true },
    }).catch(() => null),
    db.query.userStats.findFirst({ where: eq(userStats.userId, userId) }).catch(() => null),
    db.select({ count: count() }).from(follows).where(eq(follows.followingId, userId)).catch(() => [{ count: 0 }]),
    db.select({ count: count() }).from(follows).where(eq(follows.followerId, userId)).catch(() => [{ count: 0 }]),
  ])

  if (!dbUser) return NextResponse.json({ error: '유저를 찾을 수 없습니다' }, { status: 404 })
  ...
  let battles: UserProfileData['battles'] = []
  ...
  try {
    ...
    }).from(betters).where(eq(betters.userId, userId))
    ...
    const [myVotes, myLikes] = ids.length
      ? await Promise.all([
          ...
          }).from(votes).leftJoin(users, eq(votes.voterId, users.id))
            .where(inArray(votes.betterId, ids)),
          db.select({ betterId: likes.betterId }).from(likes).where(inArray(likes.betterId, ids)),
        ])
      : [[], []]
    ...
    for (const l of myLikes) likeCountMap.set(l.betterId, (likeCountMap.get(l.betterId) ?? 0) + 1)
    ...
    console.error('[/api/admin/profile]', (e as Error)?.message)
  }
