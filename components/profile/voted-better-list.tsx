...
import type { VotedBattle } from '@/app/api/user/profile/voted/route'
...
function VotedBattleCard({ battle }: { battle: VotedBattle }) {
  ...
  if (!hasWinner) {
    resultLabel = t('profile.votedCounting')
    resultColor = '#9CA3AF'
  } else if (battle.winner === battle.myChoice) {
      ...
      <div style={{
        ...
        <div style={{ padding: '10px 10px 14px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
            ...
            <span style={{ fontSize: '0.78rem', fontWeight: 700, marginLeft: 'auto', color: resultColor }}>
              {resultLabel}
            </span>
          </div>
...
export function VotedBetterList() {
  const t = useTranslations()
  const [battles, setBattles] = useState<VotedBattle[]>([])
  const [status, setStatus] = useState<'loading' | 'done' | 'error'>('loading')

  useEffect(() => {
    fetch('/api/user/profile/voted')
      .then(r => r.json())
      .then((data: VotedBattle[]) => {
        setBattles(data)
        setStatus('done')
      })
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      {battles.map(battle => (
        <VotedBattleCard key={battle.id} battle={battle} />
      ))}
    </div>
  )
}
